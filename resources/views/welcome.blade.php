...
@section('body_content')
    ...
    <section class="px-6 md:px-20 mt-6">
        ...
        @php
            $randomProducts = $products->shuffle()->slice(0, 8);
        @endphp
    ...
    <section class="px-6 md:px-20 mt-10 mb-6">
        <div class="coupon-carousel owl-carousel flex flex-wrap gap-6">
            @foreach ($coupons as $item)
                <div class="bg-white rounded-md shadow mb-2 flex justify-between items-center">

                    <div class="flex flex-col pl-3 pr-3 py-1">
                        <span class="text-gray-400 leading-5">New Coupon</span>
                        <strong class="text-rose-500">{{ $item->code }}</strong>
                    </div>

                    <div class="bg-red-400 w-12 font-medium text-white p-3 rounded-r-md">
    ...
    <section class="px-6 md:px-20 mt-8">
        <div class="flex items-center justify-between">
            <div class="flex gap-2">
                ...
                <h3 class="text-gray-800 font-medium mb-2">New Arrivals</h3>
            </div>
            <a href="{{ route('products') }}" class="text-violet-600 font-medium mb-2">All Products</a>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            @foreach ($products->slice(0, 8) as $item)
                <x-product.card1 :product="$item"/>
            @endforeach

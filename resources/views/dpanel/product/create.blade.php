...
@section('body_content')
    ...
    <form action="{{ route('dpanel.product.store') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-y-2 gap-x-4">
                ...
                <div class="md:col-span-3">
                    ...
                    <textarea name="description" rows="3" placeholder="Enter Product Description" 
                        class="w-full bg-transparent border border-gray-700 rounded py-0.5 px-2 focus:outline-none"></textarea>
                </div>
                ...
                <div>
                    <label>Stocks</label>
                    <input type="number" name="stock" placeholder="Enter Available Stocks"
                            class="w-full bg-transparent border border-gray-700 rounded py-0.5 px-2 focus:outline-none" required>
                </div>
            </div>
        </section>
        ...
        <section class="bg-white px-3 pb-3 rounded mb-3">
            ...
            <div id="image_container" class="grid grid-cols-1 md:grid-cols-8 gap-3">
                <div class="relative">
                    <label for="img-1"
                        class="flex items-center justify-center bg-white rounded-md shadow-md p-1 cursor-pointer">
                        <input type="file" id="img-1" name="images[]" onchange="setImagePreview(this, event)" accept="image/*" class="hidden" required>
                        <img src="https://placehold.jp/400x400.png?text=Add%20Image" 
                            class="rounded-md aspect-[2/2] object-cover" alt="">
                    </label>
                </div>
        ...
        <button class="bg-rose-500 text-center text-white font-medium w-full py-1 rounded shadow-md uppercase">Add New Product</button>
